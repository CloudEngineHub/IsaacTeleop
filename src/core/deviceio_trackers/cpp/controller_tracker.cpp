...
const Serialized<ControllerSnapshotTracked>& ControllerTracker::get_left_controller(const ITrackerSession& session) const
{
    return static_cast<const IControllerTrackerImpl&>(session.get_tracker_impl(*this)).get_left_controller();
}

const Serialized<ControllerSnapshotTracked>& ControllerTracker::get_right_controller(const ITrackerSession& session) const
{
