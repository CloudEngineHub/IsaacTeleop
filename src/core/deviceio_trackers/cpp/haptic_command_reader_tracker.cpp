...
const Serialized<HapticCommandTracked>& HapticCommandReaderTracker::get_data(const ITrackerSession& session) const
{
    return static_cast<const IHapticCommandReaderTrackerImpl&>(session.get_tracker_impl(*this)).get_data();
}

const Serialized<HapticCommandTracked>& HapticCommandReaderTracker::get_data(const ITrackerSession& session,
