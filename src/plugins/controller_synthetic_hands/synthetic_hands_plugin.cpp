...
void SyntheticHandsPlugin::worker_thread()
{
    ...
    while (m_running)
    {
        core::Serialized<core::ControllerSnapshotTracked> left_tracked;
        core::Serialized<core::ControllerSnapshotTracked> right_tracked;
        try
        {
        ...
        if (left_tracked && left_tracked->data() != nullptr)
            left_target = left_tracked->data()->inputs()->trigger_value();
        if (right_tracked && right_tracked->data() != nullptr)
        ...
        if (m_left_enabled && left_tracked && left_tracked->data() != nullptr)
        {
        ...
        if (m_right_enabled && right_tracked && right_tracked->data() != nullptr)
        {
            bool grip_valid = false;
            bool aim_valid = false;
