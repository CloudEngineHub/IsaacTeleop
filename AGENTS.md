...
- In sandboxed macOS environments, REUSE multiprocessing can fail while reading
  `SC_SEM_NSEMS_MAX`; rerun the full hook set outside the sandbox before treating
  that `PermissionError` as a source failure.
