...
The only test here that touches hardware, and it skips loudly without it -- see
the note in ../CMakeLists.txt about why the rest do not. What it buys is the two
conversions that are otherwise invisible until someone is wearing a headset: the
y flip (OpenGL renders bottom-up, XR wants top-down) and the depth inversion
(mjr_render is reverse Z, ProjectionLayer is told near -> 0).

It does NOT cover ProjectionLayer.submit, the XR swapchain copy, or whether the
...
def test_row_zero_is_the_top_of_the_operators_view(rendered):
    """The y flip. OpenGL renders bottom-up and XR swapchains are top-down, so
    without the flip in the readback shader the whole scene is upside down --
    which nothing short of a headset would show."""
    render, read = rendered
...
def test_depth_is_the_standard_z_projection_layer_is_promised(rendered):
    ...
    mjr_render writes the reverse of this. Getting it backwards leaves colour
    perfect and reprojection inverted, so the depth values are checked against
    the geometry that produced them, not merely for being in range.
    """
    render, read = rendered
    render([0.0, 0.0, -GHOST_DISTANCE])
...
def test_the_eyes_see_the_object_at_different_offsets(rendered):
    """Stereo parallax, and its sign. An object in front of the operator sits
    further left in the RIGHT eye; swapping the two views inverts the depth cue
    and reads as eye strain rather than as a bug."""
    render, read = rendered
