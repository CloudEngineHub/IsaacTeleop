...
def _default_scene():
    """The shipped default scene, which is the one that includes the ghost.

    Skips on an unfetched checkout: the meshes come from
    scripts/fetch-so-arm.sh, and saying so beats an "Error opening file".
    """
...
def test_both_ghost_bodies_are_mocap_and_kinematic():
    ...
    The trigger is a second mocap body rather than a jointed child so it can
    ...
    for body in bodies:
        ...
        # No joint anywhere on the ghost. A jointed child of a mocap body
        # compiles and honours a written qpos, but mj_step then integrates
        # gravity into it -- see the comment in leader_gripper.xml.
...
def test_the_leader_meshes_are_scaled_from_millimetres():
    ...
    The camera ends up inside a 65 m solid. The servo is deliberately absent
    from this list: it is authored in metres and carries no scale.
    """
    model = _default_scene()
...
def test_squeezing_drives_the_jaw_from_released_to_squeezed():
    ...
    Asserted on the recovered hinge ANGLE, not on where a point ends up: over
    a large sweep a point on the lever traces an arc, so its position along any
    fixed axis rises before it falls.
    """
    model = _default_scene()
    data = mujoco.MjData(model)
    ghost = app._resolve_ghost(model)
    ...
    def hinge_angle_at(closedness):
        ...
        # Signed against the hinge axis, so a rotation the wrong way reads
        # negative rather than folding onto the same magnitude.
...
def test_the_shipped_retargeter_drives_the_jaw_channel():
    ...
    retargeter rather than this file's idea of it.
    """
