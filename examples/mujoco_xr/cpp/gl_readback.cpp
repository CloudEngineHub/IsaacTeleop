...
// gl_VertexID -> a single triangle covering the viewport, so there is no vertex
// buffer to own. uv is in [0,1] with the GL origin (bottom-left).
...
// A pack buffer plus its CUDA registration. ReadOnly because CUDA only ever
// reads it: ProjectionLayer.submit() copies out of the pointer.
...
// The depth format `src_fbo` was built with, as the (internal, format, type)
// triple a matching texture needs.
struct DepthFormat
{
...
void Readback::capture(uint32_t view, GLuint src_fbo)
{
    ...
    View& v = views_[view];
    // glReadPixels below writes the pack buffers, which is graphics access and
    // is illegal while CUDA holds them mapped.
    unmap(v, /*throw_on_error=*/true);

    // mjr_render draws into whatever is bound when it is called -- it binds
    // nothing itself, and only calls mjr_restoreBuffer on the way out. Leaving
    // our own framebuffer (or 0) bound sends the NEXT frame to it, and the
    // symptom is an empty image with no GL error anywhere.
    const ScopedDrawFramebuffer restore_binding;
    ...
    // MuJoCo leaves depth test, culling, blending and scissor on; none of them
    // may touch a fullscreen pass whose only job is to move pixels. Not
    // restored afterwards: mjr_render calls initGL3 every frame, which sets all
    // four again.
    ...
    // Tight rows: VizBuffer's default pitch is width * bytes_per_pixel, and the
    // 4-byte default alignment would pad an odd-width RGBA8 row.
...
void Readback::map()
{
    for (View& v : views_)
    {
        ...
        // cudaGraphicsMapResources orders itself after the GL work already
        // issued on this thread, so the glReadPixels above needs no glFinish.
        cudaGraphicsResource_t resources[2] = { static_cast<cudaGraphicsResource_t>(v.color_resource),
                                                static_cast<cudaGraphicsResource_t>(v.depth_resource) };
...
void Readback::destroy()
{
    for (View& v : views_)
    {
        ...
        // GL deletes only if the entry points were ever resolved; the caller
        // owns the ordering (Renderer.close() before mujoco.GLContext.free()).
        if (loaded())
        {
