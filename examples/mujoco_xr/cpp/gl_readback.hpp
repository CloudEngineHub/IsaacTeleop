...
// viz::ProjectionLayer.submit() consumes, without a host round trip. README.md
// has the stage-by-stage version.
//
...
private:
    ...
    void build_program();
    // throw_on_error false on the teardown path: destroy() runs from a
    // destructor, and it is reached with the GL context already gone whenever
    // an exception unwound the frame loop.
