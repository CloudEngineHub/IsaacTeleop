...
// geoms than this", a hard error rather than something to tune at runtime, and
// 20k is ~30x what a tabletop scene produces.
...
// physical device by UUID, so "viz's GPU" is just the current CUDA device.
// The GL context is created independently (mujoco.GLContext), and nothing makes
// it land on the same card.
...
// Rotates a direction from the XR reference frame into MuJoCo world. Rotation
// only, so mj_from_xr's workspace translation must not appear here.
...
SceneRenderer::SceneRenderer(const Config& config, const mjModel* model) : config_(config)
{
    ...
    // Validates near/far once, up front, on a fov that is certainly non-degenerate.
    ...
    context_made_ = true;
    // Sizes the offscreen buffer to the eye buffer regardless of what the scene
    // XML declared in <visual><global offwidth/offheight>.
...
void SceneRenderer::render(const std::vector<float>& poses_xyz_qwxyz, const std::vector<float>& fovs_lrud)
{
    ...
    for (size_t v = 0; v < n; ++v)
    {
        ...
        // The eye pose crosses into MuJoCo world, not the geometry the other
        // way: mjr_render draws MuJoCo world and takes a camera, not a view
        // matrix.
