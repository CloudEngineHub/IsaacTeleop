...
.. warning::
   ...
   ``start-gateway.sh`` defaults to binding ``0.0.0.0:9002`` with the placeholder token ``0000``.
   The gateway refuses a non-loopback bind unless ``MOTORBRIDGE_WS_TOKEN`` is set, so that token is
   the only thing between your network and a moving arm. Outside an isolated bench network,
   override both:

   .. code-block:: bash

      MOTORBRIDGE_WS_TOKEN=<token> BIND=127.0.0.1:9002 ./start-gateway.sh
...
#. **Two gateways.** A gateway owns exactly one bus, so each arm needs its own on its own port —
   ``BIND=127.0.0.1:9002 ./start-gateway.sh --channel can0`` alongside
