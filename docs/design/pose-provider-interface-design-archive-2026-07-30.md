...
# Pose Provider — Interface Design (Archived 2026-07-30)
...
> **[[reviewer]] 2026-07-30 09:29:09 PDT — NEEDS SCOPE CORRECTION**
>
> This compatibility promise is broader than the design can support. The document later says
> collaborative topology and its interface shape are intentionally undecided, and other
> deferred items explicitly require new APIs/types (`predict_to()`, `ImageSet`, and possibly
> `slam_pose`). Those additions may require backend or consumer changes. Limit the promise to
> the seams actually stabilized by v1 rather than claiming every deferred item is
> non-breaking.
>
> Also, making Phase 2 part of v1 gives v1 an acceptance dependency on `IImu` and built-in
> fan-out, neither of which is currently specified by the WIP capture interface. Under the
> agreed strict-v1 scope, Phase 1 should be the shippable v1 and Phase 2 a follow-on migration,
> unless this document names the upstream interfaces and verification criteria that must land
...
> **[[reviewer]] 2026-07-30 09:29:09 PDT — NEEDS DIAGRAM UPDATE**
>
> The diagram has regressed from the contracts below it: it shows a nonexistent v1 `IImu`,
> omits `on_stereo_pair()`, and omits the component that owns fusion-time ordering. Show
> adapter input(s) entering either the single-adapter ordering stage or
> `SensorMultiplexer`, followed by **one** arrow carrying fusion-time-ordered owned
> `ImageFrame` / atomic stereo-pair / `ImuSample` work items into `IPoseProvider`. The Phase-1
> OAK source and future generic capture source can be alternate producers on the left; they
> should not make the estimator seam look different.
...
> **[[reviewer]] 2026-07-30 09:37:58 PDT — NEEDS SAMPLE/POSE PROVENANCE**
>
> The normalized samples dropped the source sequence/capture identifier discussed earlier,
> so neither the reorder/drop diagnostics nor a recorded pose can identify the exact source
> frame or stereo pair that produced it. Preserve the device sequence number (and one capture
> group id for a stereo pair) through `ImageFrame` and provider input. Then define which
> frame supplies `PoseEstimate.sample_time_local_ns` for stereo, where left/right have the
> same device timestamp but can have different host-local timestamps. A configured reference
> camera or an explicit capture-group timestamp would make this deterministic and testable.

> **[[reviewer]] 2026-07-30 09:29:09 PDT — NEEDS CONCURRENCY CONTRACT**
>
> “Enqueue only” does not define who serializes callbacks. Phase 1 polls all queues on one
> thread, but Phase 2 can receive camera and IMU callbacks concurrently, and mux input slots
> may likewise be written concurrently. State explicitly whether an `ISensorSink` instance
> may receive concurrent `on_image()` / `on_imu()` / `on_stereo_pair()` calls. Either require
> upstream serialization into one fusion stream before this boundary, or require every sink
> implementation to be thread-safe. Also pin the legal lifecycle interval for callbacks
> (`start()` completed through adapter quiescence before `stop()`). Atomic stereo delivery
> alone does not define ordering relative to a concurrent IMU callback.
...
> **[[reviewer]] 2026-07-30 09:37:58 PDT — BLOCKING FOR FIXED ALIGNMENT: FORBID SILENT MAP EPOCHS**
>
> Removing the public `reset()` call does not prevent a backend from internally abandoning a
> map and initializing a new world origin after tracking loss. `se3_tracker` carries no map
> epoch, so a fixed `T_session_slamworld` would then silently become wrong. Add a provider
> invariant: within one `start()`/`stop()` run, relocalization must return poses in the same
> SLAM world frame; if that frame cannot be preserved, remain `Lost`/fail and require a full
> session restart. Add a verification case with a simulated backend map-reset attempt.
...
> **[[reviewer]] 2026-07-30 09:29:09 PDT — NEEDS REFERENCE-BACKEND CAPABILITY CORRECTION**
>
> `requires_imu=true` conflicts with both this document's “optional IMU” v1 topology and
> NVIDIA's documented cuVSLAM stereo mode without an IMU (for example, the
> [RealSense tutorial](https://nvidia-isaac-ros.github.io/concepts/visual_slam/cuvslam/tutorial_realsense.html)
> selects stereo tracking mode for a D435 without IMU). Confirm the exact target cuVSLAM
> version, then declare its actual supported input modes. If IMU is optional, `requires_imu`
> must be false and backend configuration/validation should distinguish visual-only from VIO;
> if this integration deliberately requires IMU, the top-level v1 topology must stop calling
> it optional for the production backend.
...
> **[[reviewer]] 2026-07-30 09:29:09 PDT — NEEDS COMPLETE IDENTITY/COVERAGE VALIDATION**
>
> Step 3 still permits zero cameras, multiple `"imu"` entries, duplicate/empty
> `sensor_id`s, and a stereo pair whose two entries identify the same stream. Step 5 also
> does not state that `RigCalibration::rig_id` must match `RigSpec::rig_id`, or that the
> resolved calibration must contain exactly one record for every configured sensor and no
> unknown records. These checks are load-bearing because whole-record merge is keyed by
> `sensor_id`; duplicates otherwise make replacement and routing ambiguous. Define a
> one-to-one config↔calibration coverage check, in addition to extrinsic-graph completeness.
...
> **[[reviewer]] 2026-07-30 09:29:09 PDT — NEEDS BOUNDED-BACKPRESSURE POLICY**
>
> A time reorder window is not a queue bound. The adapter, mux, and provider all enqueue
> owned image buffers, but the design defines no maximum entries/bytes and no overflow
> policy. If SLAM falls behind capture, “enqueue only” therefore means either unbounded
> memory growth or an implementation-specific drop. Add bounded queue configuration and a
> deterministic policy: stereo is dropped only as a complete pair; image supersession/drop
> is counted; IMU overflow/gaps are surfaced and force the provider out of `Tracking` within
> its tolerance. Include both the direct Phase-1 path and mux path in verification.
...
> **[[reviewer]] 2026-07-30 09:29:09 PDT — BLOCKING: CANONICAL CONVENTION IS NOT PINNED**
>
> The prose says the convention is pinned, but the struct leaves it unresolved and the Open
> questions section explicitly defers it. `std::array<double,16>` does not specify storage
> order; `T_body_cam` names transform direction but not axes/handedness; model-specific
> intrinsic and distortion-vector ordering is undefined; and `time_offset_ns` has no sign
> convention. A backend-neutral interface cannot be implemented or tested without those
> definitions. Pin the strict-v1 camera models and parameter order, matrix layout, body/camera
> axes and handedness, transform notation, temporal-offset equation/sign, and rolling-shutter
> readout direction before accepting the interface. Backend-native mappings can remain
> implementation work.
...
> **[[reviewer]] 2026-07-30 09:29:09 PDT — NEEDS TIMEOUT WIRE SEMANTICS**
>
> The silence branch is not implementable as written: a synthesized invalid
> `Se3TrackerPose` still needs a pose and both timestamps for `push_buffer()`. Define whether
> the timeout emits once per silence episode or every tick, what pose filler it carries, and
> which timestamps it uses. In particular, do not fabricate a new device timestamp from the
> host clock. Add a verification case proving a silent provider clears validity without
> creating a misleading measurement time in the recording.
...
> **[[reviewer]] 2026-07-30 09:29:09 PDT — NEEDS VERIFICATION FIX**
>
> The named test configuration cannot exercise the following assertion: the stub declares
> `requires_imu=false` and is specified to emit `Tracking`, so starving its IMU queue has no
> required effect. Either make the stub scriptable with a VIO/IMU-required mode and explicit
> state transitions, or run the starvation test against each IMU-required backend. The stub
> should also be able to become silent so the `pose_timeout_ns` path above is testable.
...
> **[[reviewer]] 2026-07-30 09:29:09 PDT — NEEDS BOUNDED SYNC FAILURE BEHAVIOR**
>
> The design claims a partial group “times out,” but configures only the sync threshold.
> DepthAI's
> [Sync node](https://docs.luxonis.com/software-v3/depthai/depthai-components/nodes/sync)
> has a separate `setSyncAttempts()` policy, whose default `-1` emits only successfully
> synchronized groups. Pin `setSyncAttempts()` (or an explicit host-side deadline),
> output-queue capacity, and the drop/health counter so a missing camera cannot cause an
> unbounded wait or silent accumulation. Keep the post-dequeue exact-timestamp check as the
> v1 atomic-pair acceptance rule.
...
> **[[reviewer]] 2026-07-30 09:29:09 PDT — PHASE-2 CONTRACT STILL ADMITS A RUNTIME WIRING BUG**
>
> The comment explicitly documents a failure that startup validation does not prevent. If
> `FanOutSensorSink` remains in this design, every downstream sink must declare stereo
> capability and the fan-out must reject incompatible wiring before hardware opens. If Phase
> 2 is moved out of strict v1 as requested above, move this class and its unresolved
> capability contract with it rather than leaving a known runtime throw in the accepted v1
> design.
